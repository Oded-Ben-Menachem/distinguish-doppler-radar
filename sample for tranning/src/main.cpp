...
void ARDUINO_ISR_ATTR onTimer(){
  ...
  sampleIndex ++;
  if (sampleIndex == Arr_size){
    if (ActiveArry == 1){
      ActiveArry = 2;
      Arry1Ready = true;
//      memcpy((void*)Arry2,(void*)&Arry1[Arr_size/2], Arr_size/2 * sizeof(uint16_t));
    }
    else{
      ActiveArry = 1;
      Arry2Ready = true;
//      memcpy((void*)Arry1,(void*)&Arry2[Arr_size/2], Arr_size/2 * sizeof(uint16_t));
    }
    sampleIndex = 0;//Arr_size/2;
  }
